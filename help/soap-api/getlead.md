...
title: getLead
feature: SOAP
description: appels SOAP getLead
...
Cette fonction récupère un seul enregistrement de prospect de Marketo, avec toutes les valeurs de champ pour les champs intégrés et personnalisés, pour un prospect identifié par la clé fournie (LeadKey). Si le prospect existe en fonction des paramètres d’entrée, les attributs d’enregistrement du prospect sont renvoyés dans les résultats.

REMARQUE : les attributs de prospect qui sont de type de données de chaîne et qui sont vides ne sont pas renvoyés dans le cadre de la réponse.
...
| leadKey->keyValue | Obligatoire | keyValue correspond à la valeur par laquelle vous souhaitez interroger le prospect. |
