...
Cette fonction exécute un prospect Marketo existant dans une campagne intelligente Marketo. La campagne intelligente doit comporter un déclencheur « Campagne demandée » avec la source d’API de service web (voir ci-dessous).

![API de service web](assets/webserviceapi.png)

Deux jeux de paramètres peuvent être utilisés. Le premier cas utilise `campaignName` + `programName` + `programTokenList`. Le `programTokenList` peut alors être vide. Le deuxième cas concerne l&#39;utilisation du `campaignId` seul. Toute autre combinaison renvoie une exception de paramètre incorrecte.
...
| --- | --- | --- |
| leadList->leadKey->keyType | Obligatoire | `keyType` vous permet de spécifier le champ par lequel vous souhaitez interroger le prospect. Les valeurs possibles sont les suivantes : `IDNUM`, `EMAIL`, `SFDCLEADID`, `LEADOWNEREMAIL`, `SFDCACCOUNTID`, `SFDCCONTACTID`, `SFDCLEADID`, `SFDCLEADOWNERID` et `SFDCOPPTYID` |
| leadList->leadKey->keyValue | Obligatoire | `keyValue` est la valeur par laquelle vous souhaitez interroger le prospect. |
| source | Obligatoire | Source de la campagne. Valeurs possibles : `MKTOWS` ou `SALES`. L’énumération est définie dans le WSDL. |
| campaignId | Facultatif lorsque `campaignName`, `programName` et `programTokenList` sont regroupés dans un site de paramètres ; dans le cas contraire, `campaignId` est obligatoire | Identifiant de la campagne. REMARQUE : une erreur de paramètre incorrecte se produit si `campaignID` et `campaignName` sont tous deux transmis. |
