...
feature: SOAP
description: Utilisez getImportToListStatus pour vérifier le statut d’importation de liste statique Marketo, les champs obligatoires, la requête et la réponse XML, le code PHP et Java, ainsi que les bonnes pratiques d’interrogation.
...
Cette fonction vérifie le statut d’un appel de fonction `importToList` spécifique.

Effectuez d’abord l’appel API `importToList`, puis interrogez `getImportToListStatus` régulièrement. N’`getImportToListStatus` pas interroger plus d’une fois par minute.
...
| listName | Obligatoire | Nom de la liste statique dans Marketo pour laquelle récupérer le statut de l’import. |
