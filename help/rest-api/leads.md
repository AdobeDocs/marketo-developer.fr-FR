...
[Référence des points de fin Leads](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Leads)
...
L’une des fonctionnalités clés de l’API Leads est la méthode Description . Utilisez Description des pistes pour récupérer une liste complète des champs disponibles pour l’interaction via l’API REST et l’API SOAP, ainsi que les métadonnées pour chacune d’elles :

* Type de données 
* Noms des API REST et SOAP
...
Obtenir des pistes par type de filtre renvoie le même type d’enregistrements, mais peut renvoyer jusqu’à 300 valeurs par page. Elle requiert le `filterType` et `filterValues` paramètres de requête.

`filterType` accepte tout champ personnalisé ou la plupart des champs couramment utilisés. Appelez le `Describe2` point de terminaison pour obtenir une liste complète des champs pouvant faire l’objet d’une recherche et pouvant être utilisés dans `filterType`. Lors de la recherche par champ personnalisé, seuls les types de données suivants sont pris en charge : `string`, `email`, `integer`. Vous pouvez obtenir les détails du champ (description, type, etc.) en utilisant la méthode décrite ci-dessus.

`filterValues` accepte jusqu’à 300 valeurs au format séparé par des virgules. L’appel recherche des enregistrements pour lesquels le champ du prospect correspond à l’un des enregistrements inclus. `filterValues`. Si le nombre de pistes correspondant au filtre de piste est supérieur à 1 000, une erreur est renvoyée : &quot;1003, trop de résultats correspondent au filtre&quot;.
...
Dans cette requête, vous voyez deux champs importants : `action` et `lookupField`.  `action` spécifie le type d’opération de la requête et peut être `createOrUpdate`, `createOnly`, `updateOnly`, ou `createDuplicate`. Si elle est omise, l’action est définie par défaut sur `createOrUpdate`.  La variable `lookupField` spécifie la clé à utiliser lorsque l’action est `createOrUpdate` ou `updateOnly`. If `lookupField` est omis, la clé par défaut est `email`.

Par défaut, la partition par défaut est utilisée. Vous pouvez éventuellement spécifier la variable `partitionName` qui ne fonctionne que si l’action est `createOnly` ou `createOrUpdate`. Pour `partitionName` pour que fonctionne comme critère de déduplication supplémentaire, il doit faire partie du type source dans les règles de déduplication personnalisées. Lors d’une opération de mise à jour, si une piste n’existe pas dans la partition spécifiée, une erreur est renvoyée. Si l’utilisateur de l’API uniquement n’est pas autorisé à accéder à la partition spécifiée, une erreur est renvoyée.

La variable `id` ne peut être inclus qu’en tant que paramètre lors de l’utilisation de la variable `updateOnly` action, en tant que `id` est une clé unique gérée par le système.

La requête doit également comporter une `input` qui est un tableau d’enregistrements de piste. Chaque enregistrement de piste est un objet JSON comportant un nombre indéfini de champs de piste. Les clés incluses dans un enregistrement doivent être uniques pour cet enregistrement et toutes les chaînes JSON doivent être codées au format UTF-8. La variable `externalCompanyId` peut être utilisé pour lier l’enregistrement de piste à un enregistrement de société. La variable `externalSalesPersonId` peut être utilisé pour lier l’enregistrement de piste à un enregistrement de personne effectuant le vente.

Remarque : lorsque vous effectuez des requêtes de suppression de prospect simultanément ou dans une succession rapide, des enregistrements en double peuvent se produire lors de l’exécution de plusieurs requêtes avec la même valeur de clé si un appel ultérieur avec la même valeur est effectué avant le premier renvoi. Vous pouvez l’éviter en utilisant la variable `createOnly`, ou `updateOnly` le cas échéant, ou en mettant en file d’attente les appels et en attendant que votre appel revienne avant d’effectuer les appels upsert suivants avec la même clé.
...
Il existe quelques règles associées au nom et `displayName` attribution de noms. L’attribut name doit être unique, commencer par une lettre et contenir uniquement des lettres, des chiffres ou un trait de soulignement. La variable `displayName` doit être unique et ne peut pas contenir de caractères spéciaux.  Une convention d’affectation de nom courante consiste à appliquer une casse mixte à `displayName` pour produire un nom. Par exemple, un `displayName` de &quot;Mon champ personnalisé&quot; produirait un nom de &quot;myCustomField&quot;.
...
La variable `fieldApiName` path spécifie le nom de l’API du champ à mettre à jour. Le paramètre d’entrée requis est un tableau contenant un seul objet de champ de piste.  L’objet de champ contient un ou plusieurs attributs.
...
Pour transmettre la variable `mkt_tok` , attribuez la valeur au membre mktToken dans un enregistrement de piste dans le paramètre d’entrée comme suit.
...
L’envoi d’un formulaire suit le modèle standard de la base de données de piste. Un seul enregistrement d’objet est transmis dans le membre d’entrée requis du corps JSON d’une requête de POST. La variable `formId` Le membre contient l’identifiant du formulaire Marketo cible.

Le paramètre facultatif `programId` peut être utilisé pour spécifier le programme auquel ajouter le prospect et/ou spécifier le programme auquel ajouter des champs personnalisés de membre du programme. If `programId` est fourni, le prospect est ajouté au programme et tous les champs de membre du programme présents dans le formulaire sont également ajoutés. Notez que le programme spécifié doit se trouver dans le même espace de travail que le formulaire. Si le formulaire ne contient pas de champs personnalisés de membre de programme et `programId` n’est pas fourni, alors le prospect n’est pas ajouté à un programme. Si le formulaire réside dans un programme et `programId` n’est pas fourni, ce programme est utilisé lorsqu’un ou plusieurs champs personnalisés de membre de programme sont présents dans le formulaire.

Dans l’enregistrement d’entrée, la variable `leadFormFields` est obligatoire. Cet objet contient une ou plusieurs paires nom/valeur qui correspondent aux champs de formulaire à renseigner.  Tous les champs spécifiés doivent être définis dans le formulaire spécifié. Le nom est le nom de l’API REST pour le champ. Notez que la variable `email` est obligatoire.

La variable `visitorData` L’objet member est facultatif et contient des paires nom/valeur qui correspondent aux données de visite de page, y compris `pageURL`, `queryString`, `leadClientIpAddress`, et `userAgentString`. Peut être utilisé pour renseigner des champs d’activité supplémentaires à des fins de filtrage et de déclenchement.
...
![Remplir l’interface utilisateur du formulaire](assets/fill_out_form_activity_details.png)

## Fusionner

Il est parfois nécessaire de fusionner des enregistrements en double et Marketo facilite cette opération via l’API Merge Leads. La fusion des pistes combinera leurs logs d’activité, programmes, campagnes et listes, ainsi que les informations CRM, et fusionnera toutes leurs valeurs de champ en un seul enregistrement. Fusionner les pistes utilise un id de piste comme paramètre de chemin d’accès, et soit une seule `leadId` comme paramètre de requête ou une liste d’identifiants séparés par des virgules dans la variable `leadIds` .
...
Pour accéder à l’ID de liste via l’interface utilisateur de Marketo, accédez à la liste. La liste `id` se trouve dans l’URL de la liste statique, `https://app-****.marketo.com/#ST1001A1`. Dans cet exemple, 1001 correspond à `id` pour la liste.
...
Le point de terminaison Get Lists by Lead Id utilise un enregistrement de piste. `id` paramètre path et renvoie tous les enregistrements de liste statique dont le prospect est membre.
...
L’appartenance au programme peut être récupérée de la même manière que les listes. Les mêmes paramètres de requête facultatifs sont disponibles lorsque vous appelez le point de terminaison Get Leads by Program Id et transmettez la variable `programId` paramètre path .

Vous pouvez éventuellement transmettre un paramètre de champ contenant une liste de noms de champ séparés par des virgules à renvoyer. Si le paramètre fields n&#39;est pas inclus dans cette requête, les champs par défaut suivants sont renvoyés : `email`, `updatedAt`, `createdAt`, `lastName`, `firstName`, `membership`, et `id`. Lorsque vous demandez une liste de champs, si un champ particulier est demandé, mais n’est pas renvoyé, la valeur est implicitement nulle.

La structure de la réponse est très similaire, car chaque élément du tableau de résultats est une piste, sauf que chaque enregistrement comporte également un objet enfant appelé &quot;membership&quot;. Cet objet d’appartenance comprend des données sur la relation du prospect avec le programme indiqué dans l’appel, indiquant toujours son `progressionStatus`, `acquiredBy`, `reachedSuccess`, et `membershipDate`. Si le programme parent est également un programme d’engagement, l’adhésion aura des membres. `stream`, `nurtureCadence`, et `isExhausted` pour indiquer sa position et son activité dans le programme d&#39;engagement.
